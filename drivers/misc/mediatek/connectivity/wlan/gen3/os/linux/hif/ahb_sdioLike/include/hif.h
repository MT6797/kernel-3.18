...
#else
#define my_sdio_disable(__lock,__task)\
{\
	if (down_trylock(&__lock)) {\
		if (__task) {\
		...
		kalSendAeeWarning("[Wlan Driver]", "%s was reentry, previous caller %s, current caller %s",__func__, __task ? __task->comm:"NULL", current->comm);\
		kalBreakPoint();\
	}\
	__task = current;\
}
