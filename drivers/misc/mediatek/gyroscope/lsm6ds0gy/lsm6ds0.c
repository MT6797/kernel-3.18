...
#define LSM6DS0_GYRO_NEW_ARCH		//kk and L compatialbe


#ifdef LSM6DS0_GYRO_NEW_ARCH
#include <gyroscope.h>
#endif
...
#define LSM6DS0_GYRO_DEV_NAME        "LSM6DS0_GYRO"
#define COMPATIABLE_NAME "mediatek,lsm6ds0g"

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id lsm6ds0_gyro_i2c_id[] = {{LSM6DS0_GYRO_DEV_NAME,0},{}};
//static struct i2c_board_info __initdata i2c_lsm6ds0_gyro={ I2C_BOARD_INFO(LSM6DS0_GYRO_DEV_NAME, 0x34)}; //0xD4>>1 is right address
struct platform_device *gyroPltFmDev;

/*----------------------------------------------------------------------------*/
static int lsm6ds0_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id); 
static int lsm6ds0_gyro_i2c_remove(struct i2c_client *client);
static int LSM6DS0_gyro_init_client(struct i2c_client *client, bool enable);

#ifndef CONFIG_HAS_EARLYSUSPEND
static int lsm6ds0_gyro_resume(struct i2c_client *client);
static int lsm6ds0_gyro_suspend(struct i2c_client *client, pm_message_t msg); 
#endif
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
struct lsm6ds0_gyro_i2c_data {
    ...
    s8                      offset[LSM6DS0_GYRO_AXES_NUM+1];  /*+1: for 4-byte alignment*/
    s16                     data[LSM6DS0_GYRO_AXES_NUM+1];
	int 					sensitivity;
...
static struct i2c_driver lsm6ds0_gyro_i2c_driver = {
    .driver = {
//        .owner          = THIS_MODULE,
        .name           = LSM6DS0_GYRO_DEV_NAME,
#ifdef CONFIG_OF
        .of_match_table = gyro_of_match,
#endif
    },
	.probe      		= lsm6ds0_gyro_i2c_probe,
	.remove    			= lsm6ds0_gyro_i2c_remove,

#if !defined(CONFIG_HAS_EARLYSUSPEND)    
    .suspend            = lsm6ds0_gyro_suspend,
    .resume             = lsm6ds0_gyro_resume,
#endif
	.id_table = lsm6ds0_gyro_i2c_id,
};
#ifdef LSM6DS0_GYRO_NEW_ARCH
static int lsm6ds0_gyro_local_init(struct platform_device *pdev);
static int lsm6ds0_gyro_local_uninit(void);
static int lsm6ds0_gyro_init_flag = -1;
static struct gyro_init_info  lsm6ds0_gyro_init_info =
    {
        .name   = LSM6DS0_GYRO_DEV_NAME,
        .init   = lsm6ds0_gyro_local_init,
        .uninit = lsm6ds0_gyro_local_uninit,
    };
#endif
/*----------------------------------------------------------------------------*/
static struct i2c_client *lsm6ds0_i2c_client = NULL;

#ifndef LSM6DS0_GYRO_NEW_ARCH
static struct platform_driver lsm6ds0_driver;
#endif

static struct lsm6ds0_gyro_i2c_data *obj_i2c_data = NULL;
...
/*----------------------------------------------------------------------------*/
static int LSM6DS0_gyro_write_rel_calibration(struct lsm6ds0_gyro_i2c_data *obj, int dat[LSM6DS0_GYRO_AXES_NUM])
{
...
/*----------------------------------------------------------------------------*/
static int LSM6DS0_gyro_ResetCalibration(struct i2c_client *client)
{
	struct lsm6ds0_gyro_i2c_data *obj = i2c_get_clientdata(client);	
...
/*----------------------------------------------------------------------------*/
static int LSM6DS0_gyro_ReadCalibration(struct i2c_client *client, int dat[LSM6DS0_GYRO_AXES_NUM])
{
    struct lsm6ds0_gyro_i2c_data *obj = i2c_get_clientdata(client);
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
static int LSM6DS0_gyro_WriteCalibration(struct i2c_client *client, int dat[LSM6DS0_GYRO_AXES_NUM])
{
	struct lsm6ds0_gyro_i2c_data *obj = i2c_get_clientdata(client);
	int err = 0;
...
#endif
		return LSM6DS0_gyro_write_rel_calibration(obj, cali);
	} 
...
static int LSM6DS0_gyro_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		GYRO_LOG("set LSM6DS0 gyro power mode:ODR 100HZ ok %d!\n", enable);
	}
...
static int LSM6DS0_Set_RegInc(struct i2c_client *client, bool inc)
{
	...
	{
		GYRO_ERR("read LSM6DS0_CTRL1_XL err!\n");
		return LSM6DS0_ERR_I2C;
	}
	else
	{
		GYRO_LOG("read  LSM6DS0_CTRL1_XL register: 0x%x\n", databuf[0]);
	}
	if(inc)
	{
...
static int LSM6DS0_gyro_SetFullScale(struct i2c_client *client, u8 gyro_fs)
{
	u8 databuf[2] = {0};    
	int res = 0;
	GYRO_FUN();     
	...
	{
		GYRO_ERR("read LSM6DS0_CTRL2_G err!\n");
		return LSM6DS0_ERR_I2C;
	}
	else
	{
		GYRO_LOG("read  LSM6DS0_CTRL2_G register: 0x%x\n", databuf[0]);
	}

	databuf[0] &= ~LSM6DS0_GYRO_RANGE_MASK;//clear 
	databuf[0] |= gyro_fs;
...
static int LSM6DS0_gyro_SetSampleRate(struct i2c_client *client, u8 sample_rate)
{
	u8 databuf[2] = {0}; 
	int res = 0;
	GYRO_FUN();    

	res = LSM6DS0_gyro_SetPowerMode(client, true);	//set Sample Rate will enable power and should changed power status
	...
	databuf[0] &= ~LSM6DS0_GYRO_ODR_MASK;//clear 
...
static int LSM6DS0_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		LSM6DS0_gyro_SetPowerMode(client, true);
	}

	if(hwmsen_read_block(client, LSM6DS0_OUTX_L_G, databuf, 6))
	{
...
#if 1
	#if 0
		obj->data[LSM6DS0_AXIS_X] = (long)(obj->data[LSM6DS0_AXIS_X]) * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
		obj->data[LSM6DS0_AXIS_Y] = (long)(obj->data[LSM6DS0_AXIS_Y]) * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
...
#else
		data[LSM6DS0_AXIS_X] = (s64)(data[LSM6DS0_AXIS_X]) * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
		data[LSM6DS0_AXIS_Y] = (s64)(data[LSM6DS0_AXIS_Y]) * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
		data[LSM6DS0_AXIS_Z] = (s64)(data[LSM6DS0_AXIS_Z]) * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000); 
#endif
	}

	sprintf(buf, "%x %x %x", data[LSM6DS0_AXIS_X],data[LSM6DS0_AXIS_Y],data[LSM6DS0_AXIS_Z]);
...
{
	ssize_t res;
	struct lsm6ds0_gyro_i2c_data *obj = obj_i2c_data;
	if (obj == NULL)
	{
...
static int LSM6DS0_gyro_init_client(struct i2c_client *client, bool enable)
{
	...
	res = LSM6DS0_gyro_SetFullScale(client,LSM6DS0_GYRO_RANGE_2000DPS);//we have only this choice
	if(res != LSM6DS0_SUCCESS) 
	{
		return res;
	}
	...
	// 
	res = LSM6DS0_gyro_SetSampleRate(client, LSM6DS0_GYRO_ODR_104HZ);
	if(res != LSM6DS0_SUCCESS ) 
	{
		return res;
	}
	res = LSM6DS0_gyro_SetPowerMode(client, enable);
	if(res != LSM6DS0_SUCCESS)
	{
		return res;
	}

	GYRO_LOG("LSM6DS0_gyro_init_client OK!\n");
	//acc setting
...
/*----------------------------------------------------------------------------*/
#ifdef LSM6DS0_GYRO_NEW_ARCH
static int lsm6ds0_gyro_open_report_data(int open)
{
...
static int lsm6ds0_gyro_enable_nodata(int en)
{
	int value = en;
	int err = 0;
	struct lsm6ds0_gyro_i2c_data *priv = obj_i2c_data;
	...
	{	
		err = LSM6DS0_gyro_SetPowerMode( priv->client, enable_status);	
	}
...
static int lsm6ds0_gyro_set_delay(u64 ns)
{
    int value =0;
	struct lsm6ds0_gyro_i2c_data *priv = obj_i2c_data;
...
static int lsm6ds0_gyro_get_data(int* x ,int* y,int* z, int* status)
{
...
#ifndef LSM6DS0_GYRO_NEW_ARCH
/*----------------------------------------------------------------------------*/
int LSM6DS0_gyro_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;	
	struct lsm6ds0_gyro_i2c_data *priv = (struct lsm6ds0_gyro_i2c_data*)self;
	hwm_sensor_data* gyro_data;
	char buff[LSM6DS0_BUFSIZE];	
       printk("===>LSM6DS0_gyro_operate cmd=%d\n", command);
	switch (command)
	{
		case SENSOR_DELAY:
...
static long lsm6ds0_gyro_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
	...
	switch(cmd)
	{
		case GYROSCOPE_IOCTL_INIT:
			LSM6DS0_gyro_init_client(client, false);			
			break;
		...
		case GYROSCOPE_IOCTL_SET_CALI:
			...
			{
				cali[LSM6DS0_AXIS_X] = (s64)(sensor_data.x);// * 180*1000*1000/(LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142);
				cali[LSM6DS0_AXIS_Y] = (s64)(sensor_data.y);// * 180*1000*1000/(LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142);
				cali[LSM6DS0_AXIS_Z] = (s64)(sensor_data.z);// * 180*1000*1000/(LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142); 			
				err = LSM6DS0_gyro_WriteCalibration(client, cali);
			}
			break;

		case GYROSCOPE_IOCTL_CLR_CALI:
			err = LSM6DS0_gyro_ResetCalibration(client);
			break;
		...
		case GYROSCOPE_IOCTL_GET_CALI:
			...
			err = LSM6DS0_gyro_ReadCalibration(client, cali);
			if(err)
			{
				break;
			}
			
			sensor_data.x = (s64)(cali[LSM6DS0_AXIS_X]);// * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
			sensor_data.y = (s64)(cali[LSM6DS0_AXIS_Y]);// * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
			sensor_data.z = (s64)(cali[LSM6DS0_AXIS_Z]);// * LSM6DS0_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000); 
...
#if 1
/*----------------------------------------------------------------------------*/
static struct file_operations lsm6ds0_gyro_fops = {
	.owner = THIS_MODULE,
	.open = lsm6ds0_open,
	.release = lsm6ds0_release,
	.unlocked_ioctl = lsm6ds0_gyro_unlocked_ioctl,
#ifdef CONFIG_COMPAT
...
/*----------------------------------------------------------------------------*/
static struct miscdevice lsm6ds0_gyro_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "gyroscope",
	.fops = &lsm6ds0_gyro_fops,
};
#endif
...
static int lsm6ds0_gyro_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		err = LSM6DS0_gyro_SetPowerMode(obj->client, false);
		if(err)
		{
...
static int lsm6ds0_gyro_resume(struct i2c_client *client)
{
	...
	err = LSM6DS0_gyro_SetPowerMode(obj->client, enable_status);
	if(err)
	{
...
/*----------------------------------------------------------------------------*/
static void lsm6ds0_gyro_early_suspend(struct early_suspend *h) 
{
	struct lsm6ds0_gyro_i2c_data *obj = container_of(h, struct lsm6ds0_gyro_i2c_data, early_drv);   
	int err;
	GYRO_FUN();    
...
/*----------------------------------------------------------------------------*/
static void lsm6ds0_gyro_late_resume(struct early_suspend *h)
{
	struct lsm6ds0_gyro_i2c_data *obj = container_of(h, struct lsm6ds0_gyro_i2c_data, early_drv);         
	int err;
	GYRO_FUN();
...
static int lsm6ds0_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct lsm6ds0_gyro_i2c_data *obj;
	...
	memset(obj, 0, sizeof(struct lsm6ds0_gyro_i2c_data));
	...
	err = LSM6DS0_gyro_init_client(new_client, false);
	if(err)
	{
...
#if 1
	err = misc_register(&lsm6ds0_gyro_device);
	if(err)
	{
		GYRO_ERR("lsm6ds0_gyro_device misc register failed!\n");
		goto exit_misc_device_register_failed;
	}
#endif

#ifdef LSM6DS0_GYRO_NEW_ARCH
	err = lsm6ds0_create_attr(&(lsm6ds0_gyro_init_info.platform_diver_addr->driver));
#else
	...
	gyro_sobj.self = obj;
    gyro_sobj.polling = 1;
    gyro_sobj.sensor_operate = LSM6DS0_gyro_operate;
...
#else
    ctl.open_report_data= lsm6ds0_gyro_open_report_data;
    ctl.enable_nodata = lsm6ds0_gyro_enable_nodata;
    ctl.set_delay  = lsm6ds0_gyro_set_delay;
    ...
    data.get_data = lsm6ds0_gyro_get_data;
	...
	obj->early_drv.suspend  = lsm6ds0_gyro_early_suspend,
	obj->early_drv.resume   = lsm6ds0_gyro_late_resume,    
	register_early_suspend(&obj->early_drv);
#endif 
#ifdef LSM6DS0_GYRO_NEW_ARCH
	lsm6ds0_gyro_init_flag = 0;
#endif
	GYRO_LOG("%s: OK\n", __func__);
	...
	misc_deregister(&lsm6ds0_gyro_device);
...
#else
	err = lsm6ds0_delete_attr(&(lsm6ds0_gyro_init_info.platform_diver_addr->driver));
#endif
	if(err)
	{
		GYRO_ERR("lsm6ds0_gyro_i2c_remove fail: %d\n", err);
	}

	#if 1
	err = misc_deregister(&lsm6ds0_gyro_device);
	if(err)
	{
		GYRO_ERR("misc_deregister lsm6ds0_gyro_device fail: %d\n", err);
	}
	#endif
...
/*----------------------------------------------------------------------------*/
#ifndef LSM6DS0_GYRO_NEW_ARCH
static int lsm6ds0_gyro_probe(struct platform_device *pdev) 
{
	struct gyro_hw *gy_hw = hw;
	GYRO_FUN();

	LSM6DS0_power(gy_hw, 1);
	
	if(i2c_add_driver(&lsm6ds0_gyro_i2c_driver))
	{
...
/*----------------------------------------------------------------------------*/
static int lsm6ds0_gyro_remove(struct platform_device *pdev)
{
    struct gyro_hw *gy_hw = hw;
	
    //GYRO_FUN();    
    LSM6DS0_power(gy_hw, 0);  	
    i2c_del_driver(&lsm6ds0_gyro_i2c_driver);
    return 0;
}
...
static struct platform_driver lsm6ds0_driver = {
	.probe      = lsm6ds0_gyro_probe,
	.remove     = lsm6ds0_gyro_remove,    
	.driver     = {
			.name  = "gyroscope",
		//	.owner	= THIS_MODULE,
	#ifdef CONFIG_OF
...
#endif
/*----------------------------------------------------------------------------*/
static int __init lsm6ds0_gyro_init(void)
{
	//GYRO_FUN();
	//struct gyro_hw *hw = hw;
   // GYRO_LOG("%s: i2c_number=%d\n", __func__,hw->i2c_num);
    //i2c_register_board_info(hw->i2c_num, &i2c_lsm6ds0_gyro, 1);
...
#else
    gyro_driver_add(&lsm6ds0_gyro_init_info);
#endif
	return 0;    
}
/*----------------------------------------------------------------------------*/
static void __exit lsm6ds0_gyro_exit(void)
{
	GYRO_FUN();
#ifndef LSM6DS0_GYRO_NEW_ARCH
...
/*----------------------------------------------------------------------------*/
module_init(lsm6ds0_gyro_init);
module_exit(lsm6ds0_gyro_exit);
/*----------------------------------------------------------------------------*/
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("LSM6DS0 Accelerometer and gyroscope driver");
