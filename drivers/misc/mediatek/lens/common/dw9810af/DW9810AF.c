...
/*
 * DW9810AF voice coil motor driver
 *
 *
 */
#include <linux/i2c.h>
...
#define AF_DRVNAME "DW9810AF_DRV"
...
*/
	*a_pu2Result = (read_data(0x03)<<8) + (read_data(0x04)&0xff);//(((u16) pBuff[0]) << 4) + (pBuff[1] >> 4);
...
{
	int ret = 0;
	printk("111111111111111 moveaf g_pAF_Opened = %d a_u4Position = %ld\n",*g_pAF_Opened,a_u4Position);
...
initdrv();
		...
		if (ret == 0) {
			printk("Init Pos %u %ld\n", InitPos,g_u4CurrPosition);
...
/* ////////////////////////////////////////////////////////////// */
long DW9810AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int DW9810AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
void DW9810AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
