...
*****************************************************************************/
#ifdef CONFIG_MTK_LEGACY
static struct i2c_board_info tps65133_board_info __initdata = {I2C_BOARD_INFO(I2C_ID_NAME, TPS_ADDR)};
#endif
...
static struct i2c_client *tps65133_i2c_client;


/*****************************************************************************
* Function Prototype
*****************************************************************************/
static int tps65133_probe(struct i2c_client *client, const struct i2c_device_id *id);
static int tps65133_remove(struct i2c_client *client);
/*****************************************************************************
* Data Structure
*****************************************************************************/
...
static const struct i2c_device_id tps65133_id[] = {
	{ I2C_ID_NAME, 0 },
	{ }
};
...
#endif
*/
static struct i2c_driver tps65133_iic_driver = {
	.id_table	= tps65133_id,
...
static int tps65133_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	pr_debug("tps65133_iic_probe\n");
	pr_debug("TPS: info==>name=%s addr=0x%x\n", client->name, client->addr);
	tps65133_i2c_client  = client;
	return 0;
}
...
static int tps65133_remove(struct i2c_client *client)
{
	pr_debug("tps65133_remove\n");
	tps65133_i2c_client = NULL;
	i2c_unregister_device(client);
	return 0;
}


int tps65133_write_bytes(unsigned char addr, unsigned char value)
{
	int ret = 0;
	struct i2c_client *client = tps65133_i2c_client;
	char write_data[2] = {0};

	if (client == NULL) {
		pr_debug("ERROR!!tps65133_i2c_client is null\n");
		return 0;
	}
	...
	if (ret < 0)
		pr_debug("tps65133 write data fail !!\n");
	return ret;
}
EXPORT_SYMBOL_GPL(tps65133_write_bytes);
...
static int __init tps65133_iic_init(void)
{
	pr_debug("tps65133_iic_init\n");
#ifdef CONFIG_MTK_LEGACY
	i2c_register_board_info(TPS_I2C_BUSNUM, &tps65133_board_info, 1);
	pr_debug("tps65133_iic_init2\n");
#endif
	i2c_add_driver(&tps65133_iic_driver);
	pr_debug("tps65133_iic_init success\n");
	return 0;
}

static void __exit tps65133_iic_exit(void)
{
	pr_debug("tps65133_iic_exit\n");
	i2c_del_driver(&tps65133_iic_driver);
}

module_init(tps65133_iic_init);
module_exit(tps65133_iic_exit);

MODULE_AUTHOR("Xiaokuan Shi");
MODULE_DESCRIPTION("MTK tps65133 I2C Driver");
MODULE_LICENSE("GPL");
...
#else
	ret = tps65133_write_bytes(cmd, data);
	if (ret < 0)
